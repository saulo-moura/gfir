...
Require_Once('admin/fncCampo.php');
...
?>
    ...
    $(document).ready(function () {
        if (($('div#geral' + cont_arq).height() - $('div#rodape' + cont_arq).height()) > $(window).height())
...
<form id="frm" name="frm" <?php echo (count($vetFile) == 0 ? '' : 'enctype="multipart/form-data"') ?> target="_self" action="<?php echo $pagina ?>?<?php echo substr(getParametro(), 1) ?>" method="post" onSubmit="return envia()" style="margin:0px;">
    ...
    if ($vetConf[$menu] != '') {
        //echo '<div style="text-align: left;">'.conHtml($vetConf[$menu]).'</div><br>';
    }
                ...
                ForEach ($Linha as $Coluna) {
                    if (is_array($Coluna)) {
                        if ($Coluna['tipo'] != 'Include') {
                            echo '<td id="'.$Coluna['campo'].'_obj" colspan="'.$Coluna['coluna'].'">';
                            ...
                            switch ($Coluna['tipo']) {
                                case 'Geral':
                                    ...
                                    if ($valor == '')
                                        $valor = $Coluna['valor'];
                                    echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="text" class="Texto" value="'.$valor.'" size="'.$Coluna['size'].'" maxlength="'.$Coluna['maxlength'].'" '.$Coluna['js'].' />';
                                    break;

                                case 'Decimal':
                                    echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="text" class="Texto" value="'.trata_html(format_decimal($row[$Coluna['campo']])).'" size="'.$Coluna['size'].'" maxlength="'.$Coluna['maxlength'].'" '.$Coluna['js'].' />';
                                    break;

                                case 'Senha':
                                    echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="password" class="Texto" size="'.$Coluna['size'].'" maxlength="'.$Coluna['maxlength'].'" '.$Coluna['js'].' />';
                                    break;
                                ...
                                case 'Data':
                                    ...
                                    echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="text" class="Texto" value="'.$valor.'" size="10" maxlength="10" '.$js.'>';
                                    break;
                                ...
                                case 'DataHora':
                                    ...
                                    echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="text" class="Texto" value="'.$valor.'" size="16" maxlength="16" '.$js.'>';
                                    break;

                                case 'Hidden':
                                    echo '<input type="hidden" id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" value="'.trata_html($Coluna['valor']).'">';
                                ...
                                case 'cmbVetor':
                                    criar_combo_vet($Coluna['vetor'], $Coluna['campo'], $row[$Coluna['campo']], $Coluna['linhafixa'], $Coluna['js']);
                                    break;
                                ...
                                case 'cmbBanco':
                                    ...
                                    criar_combo_rs(execsql($Coluna['sql']), $Coluna['campo'], $valor, $Coluna['linhafixa'], $Coluna['js'], $Coluna['style']);
                                    break;
                                ...
                                case 'FixoBanco':
                                    ...
                                    if ($_GET[$id_fb.$Coluna['num_id']] > 0) {
                                        ...
                                        echo '<input id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" type="hidden" value="'.trata_html($_GET[$id_fb.$Coluna['num_id']]).'">';
                                ...
                                case 'Checkbox':
                                    echo '<input type="checkbox" id="'.$Coluna['campo'].'" name="'.$Coluna['campo'].'" value="'.$Coluna['valor'].'"><label for="'.$Coluna['campo'].'" class="Tit_Campo_Obr">&nbsp;'.$Coluna['descricao'].'</label>';
                                    break;
                                ...
                                case 'File':
                                    ...
                                    if (isset($row[$Coluna['campo']])) {
                                        ...
                                        ImagemProd(100, $path, $row[$Coluna['campo']], $_GET['id'].'_'.$Coluna['campo'].'_');
    ...
    function bt_codigo($top) {
        ...
        switch ($acao) {
            ...
            case 'alt':
                echo '
                    <input type="submit" name="btnAcao" class="Botao" value="Alterar" onClick="valida = \'S\'">
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                ';
                break;
        ...
        if ($botao_volta == '')
            ...
            echo "<input type='Button' name='btnAcao' $id_bt class='Botao' value='Voltar' onClick=\"$botao_volta\">";

        echo '</td><td align="right" nowrap>'.nl();
        echo '<div align="right">( * ) campo obrigatório</div>';
        echo '</td></tr>'.nl();
        echo '</table>'.nl();
