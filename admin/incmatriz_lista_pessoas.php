...
if ($idt_eap!="")
{
    /*
    $sqll   = 'select ';
    $sqll  .= '  idc_oee.idt as idc_oee_idt, fpg_fpl.codigo as  fpg_fpl_codigo ';
    $sqll  .= '  from  idc_organizacao_empresa_elemento  idc_oee ';
    $sqll  .= '  inner join fpg_departamento  fpg_fpl on fpg_fpl.idt_organizacao = idc_oee.idt ';
    $sqll  .= '  where idc_oee.idt_eap   = '.null($idt_eap);
    $rsl    = execsql($sqll);
    ForEach($rsl->data as $rowl)
    {
        $codigo_secao= $rowl['fpg_fpl_codigo'];
    }
    */
}
...
$sqll  .= '    and sca_os.codigo         = '.aspa($codigo_secao);
$rsl    = execsql($sqll);
ForEach($rsl->data as $rowl)
{
    $descricao_secao        = $rowl['descricao'];
}
...
$sqll  .= '  where sca_os.idt_organizacao = '.null($idt_organizacao);
$sqll  .= '    and sca_os.codigo = '.aspa($codigo_secao);
$sqll  .= '  order by sca_ca_descricao, sca_op.nome  ';
$rsl    = execsql($sqll);
...
echo "   <td colspan='4' class='contrato_cab_td' style='font-size:20px; background:#600000; text-align:center;' >SEÇÃO: {$descricao_secao}</td> ";
echo "</tr>";



echo "<tr class='contrato_cab_tr'>  ";
echo "   <td class='contrato_cab_td' style='width:100px; background:#0000ff; text-align:center; ' ><span style='font-size:18px;'>&nbsp;Chapa</span></td> ";
...
//echo "   <td class='contrato_cab_td' style='background:#0000ff; text-align:left; ' ><span style='font-size:18px;'>&nbsp;Custo(R$)</span></td> ";
echo "</tr>";
$codigo_secaow="#";
ForEach ($vetNomFunc as $cpf => $nome)
{
        ...
        $cargo =  $vetFuncCargo[$cpf];
        echo "   <td class='contrato_cab_td' style='background:#FFFFFF; color:#000000; text-align:left; padding-right:20px; ' >&nbsp;$cargo</td> ";
        $custo  =  $vetCusto[$cpf];
        $custow =  format_decimal($custo);
  //      echo "   <td class='contrato_cab_td' style='background:#FFFFFF; color:#000000; text-align:left; padding-right:20px; ' >&nbsp;$custow</td> ";
        echo "</tr>";
}
...
echo "<tr class='contrato_cab_tr'>  ";
echo "   <td colspan='3' class='contrato_cab_td' style='font-size:20px; background:#600000; text-align:center;' >TOTAL DE COLABORADORES: {$totalgeral} </td> ";
//echo "   <td class='contrato_cab_td' style='background:#600000; color:#FFFFFF; font-size:20px; text-align:left; padding-right:20px; ' >&nbsp;$custogeralw</td> ";
echo "</tr>";
...
function ChamaDepto(codigo_secao)
{
    ...
    var href='conteudo_matriz_pessoas.php?prefixo=inc&menu=matriz_lista_pessoas&print=n&titulo_rel=Colaboradores Associadas a seção ';
    var  left   = 0;
    var  top    = 0;
