...
<body style="background:#ffffff; width:1000px;">
...
<?php
 ...
 echo ' <div id="barra_topo_m"> ';
	...
	{
	    $idt_unidade_regional_sel=0;
	}
   // echo " ----------------------------------- ".$idt_unidade_regional_sel;    
	echo ' <div id="retornar" style="xwidth:80px;"> ';
    ...
    echo "<div  style=''>";
    echo "<center>";
    //$titulo_modulo="FUNIL DE ATENDIMENTO - RELATÓRIO";
	
	if ($idt_unidade_regional_sel>0)
	{
		$titulo_modulo= "Relatório Gerencial do Funil de Atendimento – UR {$nome_unidade_regional_sel}";
	}
	else
	{
		$titulo_modulo= "Relatório Gerencial do Funil de Atendimento do Sebrae Bahia";
	}	
...
echo '<div class="Meio" id="Meio">';
    $Require_Once="incfunil_relatorio.php";
...
?>
<script>
function MigraPdf(idt_unidade_regional)
{
   var idt_unidade_regionalw = idt_unidade_regional;
   if (idt_unidade_regionalw==0)
   {
       idt_unidade_regionalw="";
   }
   var link = "conteudo_pdf_funil.php?prefixo=inc&menu=funil_relatorio&idt_unidade_regional="+idt_unidade_regionalw;
   
	var left = 0;
	var top  = 0;
	var height = screen.height;
	var width  = screen.width;
	FunilRelatorio = window.open(link, "FunilRelatorio", "left=" + left + ",top=" + top + ",width=" + width + ",height=" + height + ",location=no,resizable=yes,menubar=no,scrollbars=yes,toolbar=no,alwaysLowered=yes,alwaysRaised=no,dependent=yes,directories=no,hotkeys=no,menubar=no,personalbar=no,scrollbars=yes,status=no,titlebar=no,z-lock=yes,titlebar=no");
	FunilRelatorio.focus();
