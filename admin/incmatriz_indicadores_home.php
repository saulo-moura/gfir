...
echo "<table id='home_indicadores'   class='' width='100%' border='0' cellspacing='0' cellpadding='0' vspace='0' hspace='0'> ";
echo "<tr class=''>  ";
...
ForEach ($rsl->data as $rowl) {
    ...
    if ($ur != $unidade_regional) {
        if ($ur != "##") {
            ...
            $indicador_4urt = 0;
            $indicador_gfn = ($indicador_1urt + $indicador_2urt + $indicador_3urt + $indicador_4urt + $indicador_5urt) / 4;
            $indicador_gf = format_decimal($indicador_gfn, 2);

            $elemento = $indicador_gfn;
            $valores_gI0 .= $virgula . $elemento;
            $elemento = aspa($ur);
            $legenda_x0 .= $virgula . $elemento;
            $virgula = ", ";

            $ele1 = format_decimal($indicador_gfn, 2);
            $ele2 = ZeroEsq($ele1, 3);
            $ele3 = $ele2 . '-' . ZeroEsq($indice, 2);
            $elemento = desformat_decimal($ele1);
            $vetValor[$ele3] = $elemento;
            $elemento = aspa($ur);
            $vetLegenda[$ele3] = $elemento;
...
$indicador_4t = 0;
$indicador_gfn = ($indicador_1t + $indicador_2t + $indicador_3t + $indicador_4t + $indicador_5t) / 4;
$indicador_gf = format_decimal($indicador_gfn, 2);

$indicador_geral = $indicador_gfn;
...
$tituloI0 = "Indicadores das Unidades Operacionais<br >Indicador Geral {$indicador_geral}";


echo "<div id='indicador0' style='width:100%;'   >";
...
$virgula = "";
$meta = 80;
//$vetajusta=explode(',',$valores_gI0);
//p($vetajusta);
ForEach ($vetValor as $idx => $Valor) {
    $elemento = $indicador_geral;
    $media_gI0 .= $virgula . $elemento;
    $meta_gI0 .= $virgula . $meta;
...
//    var idt_organizacao = <?php echo $idt_organizacao; ?>;
    ...
    $(document).ready(function () {
        ...
        chartM0 = new Highcharts.Chart({
            ...
            title: {
                margin: 50,
                text: '<?php echo $tituloI0 ?>'
            },
            xAxis: {
                categories: [<?php echo $legenda_x0 ?>],
            ...
            yAxis: {
                ...
                //reversed: true,
                title: {
                    text: '(em Quantidade de Atendimentos de Balcão)',
