...
echo "<style>";
echo "  div#topo_painel_p { ;";
echo "     border    :1.2em solid {$corpainel}; ";
echo "     font-size :1em; ;  ";
echo "     background:{$corfundopainel}; ;  ";
echo "     width:99%;  ";
echo "     display:block;  ";
echo "  } ";

echo "  .titulo_painel_p { ";
echo "     font-size :3.5em; ";
echo "     width:55%; ";
echo "     text-align:center; ";
echo "     font-weight: bold; ";
echo "     color:{$corpainel}; ";

echo "  } ";

echo "  div#ele_topo_p { ";
echo "     background:{$corpainel}; ";
echo "     color:#FFFFFF; ";
echo "     font-size :2em; ";
//echo "     width=2em; ";
echo "     padding:0.2em; ";
...
echo "     border-right:1px solid #C0C0C0; ";
echo "     padding:2px; ";

echo "  } ";


echo "  .usuario_ativo_cab_e { ";
echo "     background:#2A5696; ";
echo "     color:#FFFFFF; ";
echo "     font-size :1.2em; ";
echo "     text-align:center; ";
echo "     font-weight: bold; ";
...
echo '<audio id="beep" src="imagens/beep.wav" autostart="false" ></audio>';
//
...
if ($vetConf['refresh_painel'] == "") {
    $vetConf['refresh_painel'] = 5;
}


//$vetConf['refresh_painel'] = 50;

$refresh_painel = $vetConf['refresh_painel'] * 1000;
...
<script type="text/javascript" >
    ...
    $(document).ready(function () {
		...
		if (pri==0)
		{
		   pri=1; 
		   numpainel  = 1;
		   RefreshPainelControle(numpainel);
		}
    ...
    function RefreshTime()
    {
        ...
        {
            // self.location = 'conteudo_atendimento_painel.php';
			numpainel  = numpainel  + 1;
			if (numpainel>numpaineis)
			{
			    numpainel  = 1;
			}
            RefreshPainelControle(numpainel);
    ...
    function Desativarefresh()
    {
        var id = '#refresh_1';
        var img = $(id);
        if (desativarefresh == 0)
        {
            desativarefresh = 1;
            img.attr('src', 'imagens/refresh.gif');
            img.attr('title', 'REFRESH ATIVADO');
        }
        else
        {
            desativarefresh = 0;
            img.attr('title', 'REFRESH DESATIVADO');
            img.attr('src', 'imagens/semrefresh.gif');
        }
    }
	
	
    function RefreshPainelControle(numpainel)
    {
        $.ajax({
            dataType: 'json',
            type: 'POST',
            url: 'ajax_atendimento.php?tipo=RefreshPainelControle',
            data: {
            ...
            success: function (response) {
                if (response.erro == '') {
                    $('#painel').html(url_decode(response.html));

                    //if (url_decode(response.senha_atual) != '') {
                       // beep.play();
                //}
                } else {
