...
<?php
	...
	//echo ' <div id="helpdesk" style="xwidth:80px;"> ';
    //echo '       <a href="'.'conteudo.php?acao=&prefixo=listar&direto=S&menu=plu_helpdesk&id=0'.'" ><img width="25" height="25" src="imagens/helpdesk.png" title="HelpDesk Sebrae"/></a>';
    //echo ' </div>  ';
	
    echo "<div  style=''>";
    echo "<center>";
    $titulo_modulo="CONSULTORIA À DISTÂNCIA - RELATÓRIO DE DEVOLUTIVA";
...
*/	
    $veio_at = $_GET['veio_at'];
	$veio    = $_GET['veio'];
    $idt_atendimento_pendencia=$_GET['veio'];	
	$acao ='alt';
 
// class="showPopWin_width"   
   
   $id   = $_GET['idt_atendimento_pendencia'];
   $idt_atendimento_pendencia = $id;
   $idt_atendimento = $_GET['idt_atendimento'];
  
   $_GET['id']         = $id;
   $_GET['acao']       = $acao;
   $_GET['deondeveio'] = 'Devolutiva Distância';
   
   $prefixo = 'cadastro';
   $menu    = 'grc_atendimento_pendencia_devolutiva';
   //
