...
$reg_pagina_esp = 100;
$btListarBtMarcarTodosMulti = false;
...
$sql .= ' where idt in (40, 47, 46, 49, 50, 2, 41, 45, 48, 54)';
...
$ListarBtSelecionar = 'Despublicar Evento';
...
$vetCampo['qtd_crm'] = CriaVetTabela('Inscritos CRM');
$vetCampo['qtd_siac'] = CriaVetTabela('Inscritos SiacWeb');
...
$sql .= ' where ';
$sql .= "     {$AliasPric}.publica_internet = 'S'";
$sql .= " and {$AliasPric}.idt_evento_situacao in (14, 15, 16, 19, 20)";
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('input.listar_cmbmulti_acao').change(function () {
            if ($(this).prop('checked')) {
                var tr = $(this).parent().parent();
                var qtd_siac = tr.find('td[data-campo="qtd_siac"]').text();
                qtd_siac = str2float(qtd_siac);
                ...
                if (qtd_siac > 0) {
                    if (!confirm('Há ' + qtd_siac + ' clientes inscritos pela Loja neste evento, deseja realemente despublicar?')) {
    ...
    function ListarBtSelecionarMultiAcao() {
        ...
        if (!confirm('Deseja DESPUBLICAR os Eventos selecionados?')) {
            return false;
        }
        ...
        $.ajax({
            dataType: 'json',
            type: 'POST',
            url: ajax_sistema + '?tipo=grc_evento_despublicacao_acao',
            data: {
