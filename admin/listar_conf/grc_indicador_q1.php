...
if ($fixaunidade == 0) {   // Todos
    ...
    /*
      if ($_SESSION[CS]['g_atendimento_relacao'] == 'PG') {
...
// $vetCampo['data_inicio_atendimento'] = CriaVetTabela('Data Regi.', 'data');
$vetCampo['indicador_1'] = CriaVetTabela('Resultado do Indicador');
...
$dt_iniw = trata_data('01/01/' . $ano_base);
$dt_fimw = trata_data('31/12/' . $ano_base);

//if ($vetFiltro['consgetadatas']['valor']=="S")
//{
$sql .= ' where ';
$sql .= " {$AliasPric}.data_atendimento >= " . aspa($dt_iniw) . " and {$AliasPric}.data_atendimento <=  " . aspa($dt_fimw) . " ";
