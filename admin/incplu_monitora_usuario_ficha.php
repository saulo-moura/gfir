...
<?php
	...
	ForEach ($rs->data as $row) {
		...
		{
		    $loginw   = "";
			$loginw  .= "<div onclick='return DetalhaLogin(".'"'.$baseB.'",'.'"'.$login.'"'.");' style='cursor:pointer; color:#0000FF;' >";
		    $loginw  .= "Iniciado";
			$loginw  .= "</div>";
			$logoutw = "";
			$qtdlogin = $qtdlogin + 1;
	...
	$qtdloginw = format_decimal($qtdlogin,0);
	echo "Quantidade de Logins: {$qtdloginw} ";
	echo "</td>  ";
	echo "</tr>  ";
...
function xDetalhaLogin(base, login)
{
    ...
    function DetalhaLogin(base, login)
    {
        ...
        //alert(' teste detalha '+base+"  "+login);
        var opcao  = "A";
        var parww  = '&base=' + base + '&login=' + login ;
        var href   = 'conteudo_monitora_usuario_ficha_log.php?prefixo=inc&menu=plu_monitora_usuario_ficha_log&print=n&titulo_rel=Ficha de Monitoramento do usuário'+parww;
        var left   = 0;
        var top    = 0;
        ...
        width      = width  - 50;
        height     = height - 50;
		//alert(' teste detalha 22222222222222 '+base+"  "+login);
        var titulo = "<div style='width:700px; display:block; text-align:center; '>Usuário: " + nome_usuario + "</div>";
        showPopWin(href, titulo, width, height, close_FichaUsuarioLog, true, top, left);
        return false;
    }
    function close_FichaUsuarioLog(returnVal) {
        //alert(returnVal);
