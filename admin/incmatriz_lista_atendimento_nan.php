...
function CriarDimensoesAtendimento(&$vetDimensao) {
    ...
    ForEach ($rsl->data as $rowl) {
        ...
        $ativo     = $rowl['ativo'];
		//p($rowl); 
		if ($descricao!='INFORMAÇÃO')
		{
            $vetInstrumento[$idt] = $rowl;
		}	
		else
		{
		    $rowl['descricao']    = "EM APROVACAO";
		    $vetInstrumento[$idt] = $rowl;
		}
    }
...
function QuantitativosAtendimento($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, &$vetDimensao, &$vetDimensaoTotal, &$TotalGeral_G, &$vetDimensaoInstrumento, &$vetDimensaoTempoI, &$vetDimensaoTempoP) {
   ...
   $condicao    = " year(grc_a.data)= 2016 and ( (grc_a.nan_num_visita = 1 and grc_ga.status_1 = 'AP') or (grc_a.nan_num_visita = 2 and grc_ga.status_2 = 'AP') ) and idt_grupo_atendimento is not null ";
    ...
    QuantitativosAtendimentoEV($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, $vetDimensao, $vetDimensaoTotal, $TotalGeral_G, $vetDimensaoInstrumento, $vetDimensaoTempoI, $vetDimensaoTempoP);
}


function QuantitativosAtendimentoEV($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, &$vetDimensao, &$vetDimensaoTotal, &$TotalGeral_G, &$vetDimensaoInstrumento, &$vetDimensaoTempoI, &$vetDimensaoTempoP) {

   //$condicao    = " ( (grc_a.nan_num_visita = 1 and grc_ga.status_1 = 'AP') or (grc_a.nan_num_visita = 2 and grc_ga.status_2 = 'AP') ) and idt_grupo_atendimento is not null ";
	
	$condicao    = " year(grc_a.data)= 2016 and ( (grc_a.nan_num_visita = 1 and grc_ga.status_1 = 'EV') or (grc_a.nan_num_visita = 2 and grc_ga.status_2 = 'EV') ) and idt_grupo_atendimento is not null ";
    ...
    $sql = "select ";
    $sql .= "   count(grc_ga.idt) as quantidade, sum(horas_atendimento) as horas_atendimento, grc_a.idt_ponto_atendimento ";
    $sql .= " from grc_atendimento grc_a ";
	$sql .= " inner join grc_nan_grupo_atendimento grc_ga on grc_ga.idt =  grc_a.idt_grupo_atendimento";
	//$sql .= " inner join grc_competencia grc_c on grc_c.idt =  grc_a.idt_competencia";
    if ($condicao != "") {
    ...
    $sql .= "  group by grc_a.idt_ponto_atendimento  ";
...
function DetalhaPA(idt_ponto_atendimento)
{
    ...
    var href='conteudo_detalha_pa_nan.php?prefixo=inc&menu=detalha_lista_atendimento_pa_nan'+parww;
    var  left   = 0;
    var  top    = 0;
