...
<?php
 ...
 echo ' <div id="barra_topo_m"> ';
	...
	*/
    ...
    echo "<center>";
    $titulo_modulo="FUNIL - EXECUÇÃO DO JOB PARA OBTER CLASSIFICAR CLIENTES";
...
echo "<style>";
echo ".funil_executa_ok { ";
echo "  background:#0000FF; ";
echo "	color:#FFFFFF; ";
echo "	font-weight: bold;  ";
echo "	font-size: 3em; ";
echo "	text-align:center; ";
echo "} ";
echo ".funil_executa_erro { ";
...
echo "</style>";
//
// Executar Migração dos dados do Siacweb
//
$vetParametro   = Array();
...
$dataprotocolow = str_replace(':','',$dataprotocolow);
$vetParametro['data_protocolo']=$dataprotocolow;

$anobasew = "2017";
echo '<div id="retcla" class="funil_executa_ok" >';
echo '</div>';		
...
if ($ret==0)
{
    echo '<div class="funil_executa_ok" id="funil_executa_ok">';
	echo "Existem problemas na execução desse JOB. Favor verificar.";
	echo "Execução NÃO Obteve SUCESSO.";
    echo '</div>';
}
else
{
    echo '<div class="funil_executa_ok" id="funil_executa_ok">';
	echo "Execução Obteve SUCESSO.";
    echo '</div>';
}
*/
echo '</div>';
...
setInterval(MostraResultado, 10000);
...
function MostraResultado()
{
    ...
    alert('Estou Ativo Cla');
	
	
	$.ajax({
	dataType: 'json',
	type: 'POST',
	url: 'ajax_grc.php?tipo=BuscaAcompanhaClassifica',
	data: {
	...
	success: function (response) {
		if (response.erro == '') {
		    ...
		    // alert('Estou Ativo Cla 555555555555555555 ');
			 objd=document.getElementById('retcla');
			 if (objd != null)
			 {
				objd.innerHTML = url_decode(response.html);
			 }
		}
		else
		{
