...
if ($_SESSION[CS]['g_id_usuario'] == 1) {
    $vetd = Array();
    $vetd[2]['campo'] = "codigo_evento";
    $vetd[2]['descricao'] = "Evento";
    $vetd[2]['cor'] = "";
    $vetd[2]['tamanho'] = "";
    $vet[] = $vetd;
}
...
if (!in_array("duracao_media_processo_cotacao_servico", $_POST['sql_groupby'])) {
    $vetd = Array();
    $vetd[1]['campo'] = "duracao_media_processo_cotacao_servico";
    $vetd[1]['descricao'] = "Duração Média dos Processos de Cotação de Serviço";
    $vetd[1]['cor'] = "";
    $vetd[1]['tamanho'] = "";
    $vet[] = $vetd;
}

if (!in_array("duracao_media_processo_cotacao_aprovacao", $_POST['sql_groupby'])) {
    $vetd = Array();
    $vetd[1]['campo'] = "duracao_media_processo_cotacao_aprovacao";
    $vetd[1]['descricao'] = "Duração Média dos Processos de Aprovação de Serviço";
    $vetd[1]['cor'] = "";
    $vetd[1]['tamanho'] = "";
    $vet[] = $vetd;
}

if (!in_array("duracao_media_atendimento_cliente", $_POST['sql_groupby'])) {
    $vetd = Array();
    $vetd[1]['campo'] = "duracao_media_atendimento_cliente";
    $vetd[1]['descricao'] = "Duração Média dos Atendimentos a Clientes, nas Prestações de Serviço";
    $vetd[1]['cor'] = "";
    $vetd[1]['tamanho'] = "";
    $vet[] = $vetd;
}
...
if ($_GET['print'] != 's') {
    ...
    $nome_relatorio = 'RELATÓRIO CONTRATAÇÃO DE SERVIÇOS';
...
$Filtro = Array();
$Filtro['rs'] = execsql($sql);
$Filtro['id'] = 'f_idt_diretor';
$Filtro['id_select'] = 'id_usuario';
$Filtro['nome'] = 'Diretor';
$Filtro['LinhaUm'] = '-- Todos --';
$Filtro['valor'] = trata_id($Filtro);
$vetFiltro['f_idt_diretor'] = $Filtro;
...
$vetOrderby['a10'] = 'Área Tematica';
$vetOrderby['a11'] = 'SubÁrea';
$vetOrderby['produto'] = 'Produto';
$vetOrderby['nome_pst'] = 'PST';
$vetOrderby['media'] = 'Valor Médio do Serviço';
...
//    $campos .= ' ord.rm_consolidado';
    ...
    if ($vetFiltro['f_idt_setor']['valor'] != "" && $vetFiltro['f_idt_setor']['valor'] != "0" && $vetFiltro['f_idt_setor']['valor'] != "-1") {
        ...
        $sql_filtro .= " from " . db_pir_gec . "gec_entidade_setor";
        $sql_filtro .= " where idt = " . null($vetFiltro['f_idt_setor']['valor']);
        $sql_filtro .= " order by descricao ";
        $rs_filtro = execsql($sql_filtro);

        $pesquisa .= '<tr><td style="width:20%; text-align: left"><strong>Setor</strong></td><td style="width:80%; text-align: left">' . $rs_filtro->data[0][0] . '</td></tr>';
