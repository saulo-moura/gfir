...
if ($_GET['pesquisa'][0] == 'S') {
    ...
    //parse_str($par, $vetPar);

    $vetPar = Array();
    $vetPar['prefixo'] = 'inc';
    $vetPar['menu'] = 'grc_nan_visita_'.num_visita;
    $vetPar['session_volta'] = $_GET['session_volta'];
    $vetPar['idt_atendimento_agenda'] = $idt_atendimento_agenda;
    $vetPar['idt_atendimento'] = $idt_atendimento;
    $vetPar['id'] = $vetPar['idt_atendimento_agenda'];
    $vetPar['aba'] = $aba_vai;
    $vetPar['pesquisa'] = $_GET['pesquisa'];
    $vetPar['acao'] = $acao;
    ...
    if ($so_aba_um === true) {
        ...
        $href = $_SESSION[CS]['grc_nan_visita_'.num_visita.'_'.$_GET['session_volta']];
    } else {
        $_GET['so_aba_um'] = 'N';

        $href = "conteudo{$cont_arq}.php?prefixo=inc&menu=grc_nan_visita_".num_visita."&session_volta=".$_GET['session_volta']."&idt_atendimento_agenda=".$idt_atendimento_agenda."&idt_atendimento=".$idt_atendimento."&id=".$idt_atendimento_agenda."&pesquisa=".$_GET['pesquisa']."&aba=".$aba_vai;
    }
...
if (num_visita == 2) {
    $vetParametros['barra_inc_ap'] = false;
    $vetParametros['barra_exc_ap'] = false;
    $vetParametros['func_botao_per'] = grc_atendimento_organizacao_per_consulta;
}
...
<script type="text/javascript">
    function parListarConf_grc_atendimento_organizacao() {
        ...
        par += '&num_visita=<?php echo num_visita; ?>';
    ...
    function grc_atendimento_cadastro_dep() {
        ...
        $.ajax({
            dataType: 'json',
            type: 'POST',
            url: 'ajax_atendimento.php?tipo=nan_visita_<?php echo num_visita; ?>_valida',
            data: {
                cas: conteudo_abrir_sistema,
                idt_atendimento: '<?php echo $idt_atendimento; ?>'
            },
    ...
    function btAcaoVoltar() {
        if (confirm('Atenção! O Atendimento não foi finalizado e as informações não foram salvas.\n\nConfirma?')) {
            self.location = '<?php echo $_SESSION[CS]['grc_nan_visita_'.num_visita.'_'.$session_volta]; ?>';
        }
    ...
    function btAcaoVoltarListar() {
        if (confirm('Atenção! O Atendimento não foi finalizado e as informações não foram salvas.\n\nConfirma?')) {
            self.location = '<?php echo $_SESSION[CS]['grc_nan_visita_'.num_visita.'_listar']; ?>';
        }
