...
$vetCampo['arq_admin'] = objCmbVetor('arq_admin', 'Chama do Admin?', true, $vetSimNao);

$sql_lst_1 = 'select id_direito, nm_direito from plu_site_direito order by nm_direito';

$sql_lst_2  = 'select d.id_direito, df.id_difu, d.nm_direito from plu_site_direito d inner join
               plu_site_direito_funcao df on d.id_direito = df.id_direito
               where df.id_funcao = '.$_GET['id'].' order by d.nm_direito';

$vetCampo['id_direito'] = objLista('id_direito', True, 'Direitos do Sistema', 'sistema', $sql_lst_1, 'plu_site_direito_funcao', 200, 'Direitos da Funзгo', 'site_funcao', $sql_lst_2, '', '', 'plu_site_direito_perfil', 'id_difu');

$vetCampo['cod_assinatura'] = objTexto('cod_assinatura', 'Cуdigo Assinatura', false, 40, 45);

$sql = 'select idt, descricao from plu_setor order by codigo';
$vetCampo['idt_setor'] = objCmbBanco('idt_setor', 'Setor Responsбvel', False, $sql);


MesclarCol($vetCampo['nm_funcao'], 9);
...
MesclarCol($vetCampo['id_direito'], 9);
MesclarCol($vetCampo['idt_setor'], 9);
...
$vetFrm[] = Frame('', Array(
    ...
    Array($vetCampo['sts_menu'], '', $vetCampo['sts_linha'], '', $vetCampo['arq_admin']),
