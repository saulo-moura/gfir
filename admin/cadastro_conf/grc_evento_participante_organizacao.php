...
$idt_atendimento_organizacao = $row['idt'];
...
$js1 = " readonly='true' style='width:9em; background:#FFFFD7;'";
...
$vetCampo['idt_cnae_principal'] = objListarCmb('idt_cnae_principal', 'gec_cnae', 'Atividade Econômica Principal', true, '640px');
...
$vetFrm[] = Frame('', Array(
    ...
    Array($vetCampo['novo_registro']),
        ), $class_frame . ' none', $class_titulo, $titulo_na_linha);
            ...
            ForEach ($vetFrm['dados'] as $nLinha => $Linha) {
                ForEach ($Linha as $Coluna) {
                    if (is_array($Coluna)) {
                        ...
                        if ($Coluna['valida'] || $Coluna['campo'] == 'cnpj') {
                            $parAT[] = $Coluna['campo'];
                        }
                    }
                }
...
$campoDE = '#' . str_replace(',', ',#', $parDE);
$campoAT = '#' . str_replace(',', '_desc,#', $parAT) . '_desc';
...
<script type="text/javascript">
    ...
    function desativaFRMorg() {
        var campo = $("<?php echo $campoDE; ?>");
        var obr = $("<?php echo $campoAT; ?>");
    ...
    $(document).ready(function () {
        ...
        if ('<?php echo $trava_tudo; ?>' != 'S') {
            ...
            if (lupa) {
                if ('<?php echo $idt_atendimento_organizacao; ?>' == '') {
                    setTimeout(function () {
                        desativaFRMorg();
                ...
                setTimeout("$('#idt_tipo_empreendimento').change();", 10);
                ...
                setTimeout(function () {
                    $('#bt_voltar').focus();
                    $('#idt_tipo_empreendimento').change();
            ...
            $('#idt_tipo_informacao_e_lista > option').each(function () {
                if (ok) {
                    ...
                    if (!ok && '<?php echo idt_evento_situacao; ?>' == '8') {
                        var campo = $("#receber_informacao_e, #telefone_comercial_e, #email_e, #telefone_celular_e, #logradouro_cep_e, #logradouro_numero_e, #logradouro_complemento_e");
                        var obr = $("#receber_informacao_e_desc, #logradouro_cep_e_desc, #logradouro_numero_e_desc");
                        func_AtivaDesativa('S', "S".split(","), campo, obr, "".split(","), "N", "N");
                        $("#receber_informacao_e").change();
                    }
                }
            });
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.erro_cep == 'S' && '<?php echo idt_evento_situacao; ?>' == '8') {
                            var campo = $("#logradouro_cep_e, #logradouro_numero_e, #logradouro_complemento_e");
                            var obr = $("#logradouro_cep_e_desc, #logradouro_numero_e_desc");
                            func_AtivaDesativa('S', "S".split(","), campo, obr, "".split(","), "N", "N");
                        }

                        if (response.siacweb_situacao !== '') {
                            $("#siacweb_situacao_e_" + response.siacweb_situacao).click();
                            $('#data_fim_atividade').val(response.data_fim_atividade);
                            $('#data_fim_atividade_fix').html(response.data_fim_atividade);
                        }
