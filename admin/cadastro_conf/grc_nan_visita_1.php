<style>
    ...
    #grc_avaliacao_devolutiva_produto_desc Td.Titulo_radio,
    #grc_formulario_ferramenta_gestao_desc Td.Titulo_radio {
        width: auto;
    }
    ...
    #grc_atendimento_tema_desc {
        width: 200px;
    }

    #grc_avaliacao_devolutiva_produto_desc {
        width: 780px;
    }
...
$href = "conteudo{$cont_arq}.php?prefixo=inc&menu=grc_nan_visita_1&session_volta=" . $_GET['session_volta'] . "&idt_atendimento_agenda=" . $idt_atendimento_agenda . "&idt_atendimento=" . $idt_atendimento . "&id=" . $idt_atendimento_agenda . "&pesquisa=" . $_GET['pesquisa'] . "&aba=3";

if ($_GET['pesquisa'][0] == 'S') {
    //$par = getParametro('menu,prefixo', false);
    //$href = "conteudo.php?prefixo=cadastro&menu=grc_nan_visita_1_cadastro&menu_origem=".$menu.$par;
...
$sql .= " order by {$orderby}";
// Carrega campos que serão editados na tela full
$vetCampox['grc_atendimento_tema'] = objListarConf('grc_atendimento_tema', 'idt', $vetCampo_full, $sql, $titulo, false, array_merge($vetPadraoLC, $vetParametros));
...
if ($nan_ap == 'S') {
    $vetParametros = Array(
        ...
        'contlinfim' => '',
        'acao_alt_con_p' => 'S',
        'func_botao_per' => grc_avaliacao_devolutiva_produto_per,
        'bt_inc_menu' => 'grc_avaliacao_devolutiva_produto',
    );
} else {
    $vetParametros = Array(
        //'comcontrole' => 0,
...
$sql = "select adp.idt as idt_avaliacao_devolutiva_produto, grc_pp.idt, grc_pp.descricao as grc_pp_descricao";
$sql .= " from grc_avaliacao_devolutiva_produto adp";
$sql .= " inner join grc_produto grc_pp on grc_pp.idt = adp.idt_produto";
$sql .= " inner join grc_avaliacao_devolutiva ad on ad.idt = adp.idt_avaliacao_devolutiva";
$sql .= " inner join grc_avaliacao a on a.idt = ad.idt_avaliacao";
$sql .= ' where a.idt_atendimento = $vlID';
$sql .= " and adp.ativo = 'S'";
$sql .= " order by grc_pp.descricao";

// Carrega campos que serão editados na tela full

$vetCampox['grc_avaliacao_devolutiva_produto'] = objListarConf('grc_avaliacao_devolutiva_produto', 'idt', $vetCampo_full, $sql, $titulo, false, array_merge($vetPadraoLC, $vetParametros), 'grc_produto');

$vetCampox['barra_interesse'] = objBarraTitulo('barra_interesse', 'INTERESSES', 'class_titulo_p_barra');
...
/*
  ...
  $vetCampo = Array();
  //Monta o vetor de Campo
  $vetCampo['data'] = CriaVetTabela('Data da Pendência', 'data');
  ...
  $sql .= " from {$TabelaPrinc} {$AliasPric}  ";
  $sql .= " left outer join plu_usuario plu_u on plu_u.id_usuario = {$AliasPric}.idt_gestor_local ";
  //
  $sql .= " where {$AliasPric}".'.idt_atendimento = $vlID';
...
$vetParametros = Array(
    'codigo_frm' => 'grc_avaliacao_devolutiva_ferramenta',
    'controle_fecha' => 'A',
    //'comcontrole' => 0,
    'barra_inc_ap' => false,
    'barra_alt_ap' => false,
...
$vetCampoLC = Array();
$vetCampoLC['ordem'] = CriaVetTabela('Prioridade');
$vetCampoLC['descricao'] = CriaVetTabela('Ferramenta');
$vetCampoLC['area'] = CriaVetTabela('Área de Conhecimento');
//$vetCampoLC[''] = CriaVetTabela('Percepção do Empresário');

$titulo = 'Caderno de Ferramenta';

$sql = "select ff.idt, adf.ordem, ff.descricao, fa.descricao as area";
$sql .= " from grc_avaliacao_devolutiva_ferramenta adf";
$sql .= " inner join grc_formulario_ferramenta_gestao ff on ff.idt = adf.idt_ferramenta";
$sql .= " inner join grc_formulario_area fa on fa.idt = ff.idt_area";
$sql .= " inner join grc_avaliacao_devolutiva ad on ad.idt = adf.idt_avaliacao_devolutiva";
$sql .= " inner join grc_avaliacao a on a.idt = ad.idt_avaliacao";
$sql .= ' where a.idt_atendimento = $vlID';
$sql .= " and adf.ativo = 'S'";
$sql .= " order by adf.ordem";

$vetCampox['grc_formulario_ferramenta_gestao'] = objListarConf('grc_formulario_ferramenta_gestao', 'idt', $vetCampoLC, $sql, $titulo, false, array_merge($vetPadraoLC, $vetParametros));
$vetCampox['barra_ferramenta'] = objBarraTitulo('barra_ferramenta', 'CADERNO DE FERRAMENTAS', 'class_titulo_p_barra');
...
$vetFrm[] = Frame('', Array(
    Array($vetCampox['barra_ferramenta']),
    Array($vetCampox['grc_formulario_ferramenta_gestao']),
        ), $class_frame, $class_titulo, $titulo_na_linha, $vetParametros);

MesclarCol($vetCampox['barra_interesse'], 3);

$vetFrm[] = Frame('', Array(
    Array($vetCampox['barra_interesse']),
    Array($vetCampox['grc_atendimento_tema'], '', $vetCampox['grc_avaliacao_devolutiva_produto']),
        ), $class_frame, $class_titulo, $titulo_na_linha, $vetParametros);
...
if ($nan_ap == 'S') {
    $vetCampo['grc_nan_visita_1_ap_bt2'] = objInclude('grc_nan_visita_1_ap_bt2', 'cadastro_conf/grc_nan_visita_1_ap_bt2.php');
