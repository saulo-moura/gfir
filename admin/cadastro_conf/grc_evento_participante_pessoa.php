...
if ($_GET['menu'] == 'grc_evento_participante_pessoa_p') {
    $idt_atendimento = $_GET['idCad'];
    ...
    $rs = execsql($sql);
    $idt_atendimento_pessoa = $rs->data[0][0];

    if ($idt_atendimento_pessoa == '') {
        $idt_atendimento_pessoa = 0;
    }
...
$sql = '';
$sql .= ' select e.gratuito, e.composto';
$sql .= ' from grc_evento e';
$sql .= ' where e.idt = ' . null($idt_evento);
$rst = execsql($sql);
$rowt = $rst->data[0];

$composto = $rowt['composto'];

$gratuito = $_GET['gratuito'];

if ($gratuito == '') {
    $gratuito = $rowt['gratuito'];
}
...
$vetCampo['nome_mae'] = objTexto('nome_mae', 'Nome da Mãe', false, 60, 120);
...
if ($veio == 'SG' || (voucher_utilizado != '' && substr(voucher_utilizado, 0, 2) != 'VE')) {
    unset($tmp['N']);
}

$js2 = "";
$vetCampo['representa_empresa'] = objCmbVetor('representa_empresa', 'Está Representando um Empreendimento Nesse Evento?', true, $tmp, '', $js2);

$js1 = " style='width:15em;'";
$js2 = " style='width:19em;'";
...
$vetCampo['potencial_personagem'] = objCmbVetor('potencial_personagem', 'Potencial Personagem?', false, $vetNaoSim, '', $js3);
$vetCampo['receber_informacao'] = objCmbVetor('receber_informacao', 'Receber informações do Sebrae?', true, $vetNaoSim, '', $js3);
...
$vetCampo['idt_tipo_informacao'] = objLista('idt_tipo_informacao', false, 'Receber Informações do Sistema', 'idt_tipo_informacao1', $sql_lst_1, 'grc_atendimento_pessoa_tipo_informacao', 185, 'Receber Informações Selecionadas', 'idt_tipo_informacao2', $sql_lst_2);
...
$vetCampo['idt_tipo_deficiencia'] = objLista('idt_tipo_deficiencia', false, 'Deficiência do Sistema', 'idt_tipo_deficiencia1', $sql_lst_1, 'grc_atendimento_pessoa_tipo_deficiencia', 185, 'Deficiência Selecionadas', 'idt_tipo_deficiencia2', $sql_lst_2);
...
$vetCampo['tipo_relacao'] = objHidden('tipo_relacao', $tipo_relacao);
...
if ($composto != 'S' && $origem_evento_tela != 'combo') {
    $vetCampo['evento_cortesia_tit'] = objBarraTitulo('evento_cortesia_tit', 'Cortesia?: *');
    ...
    $vetCampo['evento_cortesia'] = objCmbVetor('evento_cortesia', 'Cortesia?', true, $tmp, '');

    $vetFrm[] = Frame('', Array(
        Array($vetCampo['evento_cortesia_tit'], $vetCampo['evento_cortesia']),
            ), $class_frame, $class_titulo, $titulo_na_linha, $vetParametros);
}

if ($MesclarCadastro) {
    $vetCampo['representa_empresa_tit'] = objBarraTitulo('representa_empresa_tit', 'Está Representando um Empreendimento Nesse Evento?: *');
...
$trava_tudo = $_SESSION[CS]['tmp'][CSU]['vetParametrosMC']['trava_tudo'];

if ($_GET['id'] != 0 && ($acao == 'con' || $acao == 'exc')) {
    $trava_tudo = 'S';
}
?>
<script>
    ...
    $(document).ready(function () {
        ...
        if ('<?php echo $trava_tudo; ?>' != 'S') {
                ...
                if ($('#cpf').val() != '') {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            filadeespera: '<?php echo filadeespera; ?>',
                            idt_atendimento: '<?php echo $idt_atendimento; ?>',
                            cpf: $('#cpf').val(),
                            voucher_numero: voucher_numero,
                            vl_evento: vl_evento,
                            idt_instrumento: '<?php echo $_GET['idt_instrumento']; ?>',
                            idt_pf: '<?php echo $idt_atendimento_pessoa; ?>',
                            idt_evento: '<?php echo $idt_evento; ?>',
                            MesclarCadastro: MesclarCadastro + 'E'
                        },
                        success: function (response) {
                            ...
                            if (isNaN(idt)) {
                                ...
                                url = url.replace('&id=<?php echo $idt_atendimento_pessoa; ?>&', '&id=' + idt + '&');
            ...
            if ($('#cpf').val() == '') {
                setTimeout('func_AtivaDesativa("S", "S".split(","), $("#cpf,#nome,#idt_segmentacao,#idt_subsegmentacao,#idt_programa_fidelidade"), $("#cpf_desc,#nome_desc"), "".split(","), "N", "N")', 10);
        ...
        objd = document.getElementById('evento_cortesia_desc');
        if (objd != null)
        {
            $(objd).css('display', 'none');
        }
        ...
        if (valida == 'S') {
            ...
            $('#idt_tipo_informacao_lista > option').each(function () {
                if (ok) {
                    ...
                    if (!ok && '<?php echo idt_evento_situacao; ?>' == '8') {
                        var campo = $("#receber_informacao, #telefone_residencial, #email, #telefone_celular, #telefone_recado, #logradouro_cep, #logradouro_numero, #logradouro_complemento");
                        var obr = $("#receber_informacao_desc, #logradouro_cep_desc, #logradouro_numero_desc");
                        func_AtivaDesativa('S', "S".split(","), campo, obr, "".split(","), "N", "N");
                        $("#receber_informacao").change();
                    }
                }
            });
            ...
            if (ok) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.erro_cep == 'S' && '<?php echo idt_evento_situacao; ?>' == '8') {
                            var campo = $("#logradouro_cep, #logradouro_numero, #logradouro_complemento");
                            var obr = $("#logradouro_cep_desc, #logradouro_numero_desc");
                            func_AtivaDesativa('S', "S".split(","), campo, obr, "".split(","), "N", "N");
                        }

                        if (response.siacweb_situacao !== '') {
                            $("#siacweb_situacao_" + response.siacweb_situacao).click();
                        }
