...
function QuantitativosAtendimento($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, &$vetDimensao, &$vetDimensaoTotal, &$TotalGeral_G, &$vetDimensaoInstrumento, &$vetDimensaoTempoI, &$vetDimensaoTempoP,$op,&$vetMensal,&$vetEst) {
    //$vetDimensao = Array();
	...
	$sql = "select ";  
	$sql .= "   count(grc_a.idt) as quantidade, sum(grc_a.horas_atendimento) as horas_atendimento, substring(grc_a.data,1,7) as anomes ";
	$sql .= " from grc_atendimento grc_a ";
	$sql .= " left  join grc_evento_participante grc_ep on grc_ep.idt_atendimento =  grc_a.idt";
    ...
    $sql .= "  group by substring(data,1,7)  ";
    ...
    // Quantidade Bruta - publico alvo
    
	$sql  = "select ";  
	$sql .= "   count(grc_a.idt) as quantidade, sum(grc_a.horas_atendimento) as horas_atendimento, ";
	$sql .= "   grc_ao.idt_porte as porte_meta, ";
	
	$sql .= "   CASE ";
	$sql .= "   WHEN grc_ao.cnpj is null THEN 'F' ";
	$sql .= "   WHEN grc_ao.idt_tipo_empreendimento = 7 THEN 'J' ";
	$sql .= "   ELSE 'J' ";
	$sql .= "   END AS 'tipo_pessoa'  ";
    ...
    ForEach ($rsl->data as $rowl) {
		...
		if ($tipo_pessoa=='J' and ($porte_meta==0 or $porte_meta=='') )
        {
            $porte_meta=999;
        }
	...
	// Quantidade Bruta - natureza
    
	$sql  = "select ";  
	$sql .= "   count(grc_a.idt) as quantidade, sum(grc_a.horas_atendimento) as horas_atendimento, ";
	$sql .= "   grc_ao.idt_tipo_empreendimento, ";
    ...
    $sql .= "  group by tipo_pessoa, grc_ao.idt_tipo_empreendimento  ";
    ...
    ForEach ($rsl->data as $rowl) {
        $tipo_pessoa              = $rowl['tipo_pessoa'];
		$idt_tipo_empreendimento  = $rowl['idt_tipo_empreendimento'];
	
	
	    $naturezaw    = "AC"; // A CLASSIFICAR
		$tipo_pessoaw = "J";
        if ($tipo_pessoa == 'J') {  // Atendimento a Empresa com CNPJ ou Produtor Rural
		...
		$quantidade         = $rowl['quantidade'];
		$horas_atendimento  = $rowl['horas_atendimento'];
		$vetEst['natureza'][$natureza]['qtd']=$vetEst['natureza'][$natureza]['qtd']+$quantidade;
		$vetEst['natureza'][$natureza]['hor']=$vetEst['natureza'][$natureza]['hor']+$horas_atendimento;
    }
...
$condicao    .= " ) ";
//	
QuantitativosAtendimento($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, $vetDimensao,$vetDimensaoTotal,$TotalGeral_G, $vetDimensaoInstrumento, $vetDimensaoTempoI, $vetDimensaoTempoP,1,$vetMensal,$vetEst);
...
QuantitativosAtendimento($vetInstrumento,$vetPontoUR,$idt_dimensao, $condicao, $vetDimensao,$vetDimensaoTotal,$TotalGeral_G, $vetDimensaoInstrumento, $vetDimensaoTempoI, $vetDimensaoTempoP,2,$vetMensal,$vetEst);
...
echo "Geral: Atendimento a Pequenos Negócios sem Glosa para Metas";
echo "</div>";
...
echo "<tr class=''>  ";
	...
	ForEach ($vetInstrumento as $idt => $row) {
        ...
        $percw = format_decimal($perc, 2).' %';
        echo "   <td class='cb_perc {$classw}' style='' >$percw</td> ";
		$desc_instrumento  = $vetInstrumentosG[$idt];
		$vetAtendimentosG['instrumento'][$idt]=$quantidade;
    }
...
$vetAtendimentosG['bruto_total']=$TotalGeral_G;
...
$metaanual = $vetMetasValorAnual['GE'];
...
$onclick = " onclick='return AbreFecha(0);'; ";
$title = " title='Clique aqui para exibir ou esconder o Gráfico' ";
echo  "<div id='grafico0' {$onclick} style=''>";
echo  "<div id='grafico0' {$title} style='padding-left:5px; border-bottom:1px solid #FFFFFF; cursor:pointer; float:left; width:100%; height:30px; background:#2A5696; color:#FFFFFF;'>";
echo  "1 - Público Sebrae";
echo  "</div>";
echo  "<div id='container_grafico0' style='display:none; float:left; width:100%; height:{$altura}px; margin-top:10px; border-top:3px solid #2A5696;'></div>";
echo "</div>";
...
$quantidadet=9;
$altura = $quantidadet * 40;
$onclick = " onclick='return AbreFecha(1);'; ";

$title = " title='Clique aqui para exibir ou esconder o Gráfico' ";
echo  "<div id='grafico1' {$onclick} style=''>";
echo  "<div id='grafico1t' {$title} style='padding-left:5px; border-bottom:1px solid #FFFFFF; cursor:pointer; float:left; width:100%; height:30px; background:#2A5696; color:#FFFFFF;'>";
echo  "2 - Atendimentos - Curva ABC";
echo  "</div>";
echo  "<div id='container_grafico1' style='display:none; float:left; width:100%; height:{$altura}px; margin-top:10px; border-top:3px solid #2A5696;'></div>";
echo "</div>";
...
$onclick = " onclick='return AbreFecha(3);'; ";
echo  "<div id='grafico3' {$onclick} style=''>";
echo  "<div id='grafico3t' {$title} style='padding-left:5px; border-bottom:1px solid #FFFFFF; cursor:pointer; float:left; width:100%; height:30px; background:#2A5696; color:#FFFFFF;'>";
echo  "3 - Unidades Regionais - Atendimento por Instrumento";
echo  "</div>";
echo  "<div id='container_grafico3' style='display:none; float:left; width:100%; height:{$altura}px; margin-top:10px; border-top:3px solid #2A5696; border-bottom:3px solid #2A5696;'></div>";
echo "</div>";


$onclick = " onclick='return AbreFecha(2);'; ";
echo  "<div id='grafico2' {$onclick} style=''>";
echo  "<div id='grafico2t' {$title} style='padding-left:5px; border-bottom:1px solid #FFFFFF; cursor:pointer; float:left; width:100%; height:30px; background:#2A5696; color:#FFFFFF;'>";
echo  "4 - Atendimentos Mensal - Quantidade";
echo  "</div>";
echo  "<div id='container_grafico2' style='display:none; float:left; width:100%; height:{$altura}px; margin-top:10px; border-top:3px solid #2A5696; border-bottom:3px solid #2A5696;'></div>";
echo  "</div>";



$onclick = " onclick='return AbreFecha(4);'; ";
echo  "<div id='grafico4' {$onclick} style=''>";
echo  "<div id='grafico4t' {$title} style='padding-left:5px; border-bottom:1px solid #FFFFFF; cursor:pointer; float:left; width:100%; height:30px; background:#2A5696; color:#FFFFFF;'>";
echo  "5 - Atendimentos por Instrumento - Geral";
echo  "</div>";
echo  "<div id='container_grafico4' style='display:none; float:left; width:100%; height:{$altura}px; margin-top:10px; border-top:3px solid #2A5696; border-bottom:3px solid #2A5696;'></div>";
echo  "</div>";
...
$vetPorte=Array();
$vetPorte[0]   = 'PF'; //Potencial Empresário

$vetPorte[1]   = 'EPP'; //Pequena Pequeno --- *

$vetPorte[2]   = 'MD';  // Média Empresa 

$vetPorte[3]   = 'ME'; //Microempresa --- *

$vetPorte[4]   = 'GE'; //Grande Empresa

$vetPorte[5]   = 'MEI'; //Micro Emprrendedor Individual --- *

$vetPorte[999] = 'AC'; //Pessoa Juridica a classificar
	...
	ForEach ($vetMetaF as $porte => $vetqtd) {
		...
		$portew = $vetPorte[$porte];
		$vetEst5['F'][$portew]=$vetEst5['F'][$portew]+$quantidade;
	}
	...
	ForEach ($vetMetaJ as $porte => $vetqtd) {
		...
		$portew = $vetPorte[$porte];
		$vetEst5['J'][$portew]=$vetEst5['J'][$portew]+$quantidade;
	}	
...
$vetLeg['PF']  = 'Pessoa Física';
$vetLeg['PP']  = 'Pequena Empresa';
$vetLeg['GE']  = 'Empresa de Pequeno Porte';
...
ForEach ($vetEst5 as $tipo_pessoa => $vetPorte) {
    ForEach ($vetPorte as $porte => $quantidade) {
		...
		$vetAtendimentosG['PS'][$porte]=$quantidade;
...
$vetNaturezaPainel=$vetEst['natureza'];
...
$(document).ready(function() {
     ...
     chartM0 = new Highcharts.Chart({
        chart: {
		    renderTo: 'container_grafico0',
            type: 'bar'
        },
        title: {
		    margin:		    50,
...
function AbreFecha(op)
{
    var id = "#container_grafico"+op;
    $(id).toggle();  
	return false;
}
