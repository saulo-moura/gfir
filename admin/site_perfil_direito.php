...
$sql = 'select id_funcao, id_direito, id_difu from plu_site_direito_funcao';
...
$sql = 'select id_difu from plu_site_direito_perfil where id_perfil = '.$_GET['id'];
$trs = execsql($sql);
...
{
    $sql  = 'select id_direito, nm_direito from plu_site_direito';
    $sql .= ' order by nm_direito ';
    echo " var veio_assinatura='S';  ";
}
else
{
    $sql  = 'select id_direito, nm_direito from plu_site_direito';
    $sql .= ' where cod_direito <> '.aspa('ASSIN');
    $sql .= ' order by nm_direito';
    echo " var veio_assinatura='N';  ";
}
//p($sql);  exit();
...
$sql = 'select id_funcao, nm_funcao, cod_classificacao from plu_site_funcao order by cod_classificacao';
$trs = execsql($sql);
$tot_funcao = $trs->rows;

ForEach($trs->data as $func) {
    $nm_funcao=alinha_nm_funcao($func['nm_funcao'],$func['cod_classificacao']);
    $nivel    =strlen($func['cod_classificacao']);
    echo "lin[lin.length] = new Array('".$func['id_funcao']."','".$nm_funcao."','".$nivel."');\n";
    ...
    ForEach($rs_direito->data as $dir) {
        ...
        if ($id_difu != '') {
            ...
            echo "val[l][c] = new Array('$id_difu','$t');\n";
        } else
...
function matriz() {
    ...
    if (fim_c>1)
    {
        td += " value='' onclick='Marca(marca)'   disabled='disabled' >&nbsp;Todos os Direitos";
    }
    else
    {
    ...
    for (l = ini_l; l <= fim_l; l++) {
        ...
        if (lin[l][2]==2)
        {
            td += " <tr><td class='Perfil_Titulo' style='font-size:16px; color:#900000' >" + lin[l][1]+ "</td>";
            dx2=" style='background:#E5E5E5; ' ";
        }
        else
        {
            td += " <tr><td class='Perfil_Titulo' style='font-size:12px; color:#3E3E3E'>" + lin[l][1] + "</td>";
            dx2='';
        }
        for (c = ini_c; c <= fim_c; c++) {
            td += " <td class='Perfil_Registro' align='center' "+dx2+" >";
