...
$sql = "select ";
$sql .= "   {$AliasPric}.*,  ";
$sql .= " ({$AliasPric}.quantidade_participante + {$AliasPric}.qtd_vagas_adicional + {$AliasPric}.qtd_vagas_extra) - ({$AliasPric}.qtd_matriculado_siacweb + {$AliasPric}.qtd_vagas_resevado + {$AliasPric}.qtd_vagas_bloqueadas) as qtd_disponivel,";
$sql .= "    i.descricao as instrumento,";
...
$sql .= " where {$AliasPric}.idt_instrumento in (2, 40, 47, 46, 49, 50, 45, 41)";

if ($includeListarCmb === true) {
    $sql .= " and {$AliasPric}.idt = " . null($includeListarCmbWhere);
} else {
    if ($_GET['par_idt_evento_origem'] != '') {
        $sql .= " and ({$AliasPric}.idt not in (";
        $sql .= ' select idt_evento';
        $sql .= ' from grc_evento_combo';
        $sql .= " where idt_evento_origem = " . null($_GET['par_idt_evento_origem']);
        $sql .= " ) or {$AliasPric}.idt = " . null($_GET['par_idt_evento']) . ")";
    }
    ...
    $sql .= " and {$AliasPric}.idt_evento_situacao in (14, 16)";
    //$sql .= " and {$AliasPric}.publica_internet = 'S'";
    $sql .= " and (gec_pr.tipo_ordem is null or gec_pr.tipo_ordem <> 'SG')";

    if ($vetFiltro['idt_instrumento']['valor'] != "" && $vetFiltro['idt_instrumento']['valor'] != "0" && $vetFiltro['idt_instrumento']['valor'] != "-1") {
        $sql .= ' and ' . $AliasPric . '.idt_instrumento = ' . null($vetFiltro['idt_instrumento']['valor']);
    }
