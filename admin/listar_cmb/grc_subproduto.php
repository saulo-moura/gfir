...
if ($includeListarCmb !== true) {
    ...
    $sql .= " where idt in (".$_GET['idt_instrumento'].')';
    $sql .= " order by codigo";
    $rs = execsql($sql);
    $Filtro = Array();
    $Filtro['rs'] = $rs;
...
$vetCampo['gec_prog_descricao'] = CriaVetTabela('Programa Credenciado');
...
$sql .= "    concat_ws(' - ', grc_pri.descricao, {$AliasPric}.descricao) as {$campoDescListarCmb}";
...
if ($includeListarCmb) {
    ...
    $sql .= " and {$AliasPric}.idt_programa_grc = ".null($_GET['idt_programa_grc']);
    $sql .= " and {$AliasPric}.idt_instrumento in (".$_GET['idt_instrumento'].')';
    $sql .= " and {$AliasPric}.idt_produto_especie = 2";
