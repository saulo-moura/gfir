...
switch ($_POST['btnAcao']) {
    case 'Confirmar':
        $sql = "select * from plu_usuario where login = ".aspa($_SESSION[CS]['g_login'])." and senha = ".aspa(md5($_POST['senha_atual']));
        $result = execsql($sql);
        $row = $result->data[0];
  ...
  //          echo "<script type='text/javascript'>alert('Usuário não esta ativo no sistema PIR!\\nPara maiores informações\\nprocurar o Administrador do sistema.');</script>";
            ...
            if ($_POST['senha_nova'] != $_POST['senha_nova_repete']) {
                ...
                if ($erro == '0') {  // comunicação de tentativa de modificação enviada
                    // modificar
                    $_SESSION[CS]['g_senha_antiga'] = md5($_POST['senha_nova']);
                    $_SESSION[CS]['g_s_slv'] = $_POST['senha_nova'];

                    WSsincronizarSenha($_SESSION[CS]['g_login'], md5($_POST['senha_nova']));

                    $sql = "select * from plu_usuario where login = ".aspa($_SESSION[CS]['g_login'])." and senha = ".aspa(md5($_POST['senha_nova']));
                    $result = execsql($sql);
                    $row = $result->data[0];
                    //
                    if ($result->rows == 0) {
...
<script>
    function sai_altera_senha() {

        self.location = 'conteudo.php?retsenha=S';
